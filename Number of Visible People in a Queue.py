...
class Solution(object):
    def canSeePersonsCount(self, heights):
        ...
        :rtype: List[int]
        """
        #iterate from left to right, store index to stack that has been iterated, wheneve current height higer than previous one,
        #remove it from stack (prvious position can see current person at i only), add one to its corresponding answer
        N=len(heights)
        stack, ans=[], [0]*N
        ...
        for i in range(N):
            while stack and heights[stack[-1]]<heights[i]:
                ans[stack[-1]]+=1
                stack.pop()
            #one can always be seen by its immediate left neighbor
            if stack:
                ans[stack[-1]]+=1
            stack.append(i)
        return ans
...
print O.canSeePersonsCount(heights)
heights = [5,1,2,3,10]
print O.canSeePersonsCount(heights)