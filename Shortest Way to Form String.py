...
#accepted but idea is from others
class Solution(object):
    def shortestWay(self, source, target):
        ...
        i, j = 0, 0
        while j < n:
            org = j  # memorize if target index has moved
            while i < m and j < n:
                if source[i] == target[j]:
                    ...
                    # move source index
                    i += 1
            if i == m and org == j:#after interation over source, target has not moved forward, means at least current char at j does not exist in source
                return -1
            if i == m or j == n:
                i = 0
                ans += 1
